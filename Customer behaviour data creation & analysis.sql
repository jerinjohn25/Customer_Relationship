create database Customer_relationship;
...
select sum(price) as Total_amount, customer_id from sales s Join menu m on s.product_id=m.product_id group by customer_id;
	
-- 2. How many days has each customer visited the restaurant?
select customer_id, Count(distinct order_date) from sales group by customer_id;

-- 3. What was the first item from the menu purchased by each customer?
select customer_id, min(order_date) as first_purchase, product_name from sales s Join menu m on s.product_id= m.product_id group by customer_id  ;
...
select Count(s.product_id) as Quantity_purchased, product_name from sales s Join menu m On s.product_id=m.product_id group by product_name order by quantity_purchased desc limit 1;
...
select s.customer_id, count(*) as total_item , sum(price)
